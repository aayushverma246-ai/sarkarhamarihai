  ...
  try {
    ...
    while (hasMore) {
      const { data, error } = await supabase
        .from('jobs')
        .select('id, job_name, organization, selection_process, salary_min, salary_max, form_status')
    ...
    console.log(`- Active jobs with generic templates: ${genericSelectionCount}`);
    console.log(`- Active jobs with zero salary: ${zeroSalaryCount}`);
