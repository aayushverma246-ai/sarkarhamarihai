...
async function run() {
  try {
    console.log('Fetching all jobs to clean up generic selection processes...');
    let allJobs = [];
    let hasMore = true;
    let page = 0;
    const pageSize = 1000;
    ...
    while (hasMore) {
      const { data, error } = await supabase
        .from('jobs')
        .select('id, job_name, organization, selection_process')
    ...
    allJobs.forEach(job => {
      ...
      if (genericTemplatesList.includes(sel)) {
        jobsToClear.push(job.id);
      }
    });
    
    console.log(`Found ${jobsToClear.length} jobs with generic selection processes.`);
    
    if (jobsToClear.length > 0) {
      console.log('Clearing selection processes for these jobs...');
      ...
      if (error) throw error;
      console.log('🎉 Successfully cleared generic templates!');
    } else {
      console.log('No generic templates remaining in the database!');
    }
