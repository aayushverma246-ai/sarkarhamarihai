...
const JobCard = React.memo(function JobCard({ job, isLiked, onLikeToggle, isApplied, onApplyToggle, onBeforeNavigate, staggerIndex = 0 }: Props) {
  ...
  return (
    <div
      ...
      className={`animate-cardIn card-hover bg-white dark:bg-[#0e0e0e] rounded-xl border p-4 cursor-pointer group transition-all duration-300 hover:shadow-2xl hover:shadow-green-500/10 group-hover:-translate-y-1 hover:-translate-y-1 ${isApplied
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2.5">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            <span className={`text-[10px] px-1.5 py-0.5 rounded border font-semibold uppercase tracking-wide ${statusStyle}`}>
              {statusLabel}
            </span>
            <span className="text-[10px] text-gray-600 bg-[#111] px-1.5 py-0.5 rounded border border-[#191919] truncate max-w-[120px]">
              {job.organization}
            </span>
            {isHighMatch && (
              <span className="text-[10px] px-1.5 py-0.5 rounded border font-semibold tracking-wide text-emerald-400 bg-emerald-900/15 border-emerald-900/25 flex items-center gap-1">
            ...
            {!isHighMatch && isEligible && (
              <span className="text-[10px] px-1.5 py-0.5 rounded border font-semibold tracking-wide text-blue-400 bg-blue-900/15 border-blue-900/25 flex items-center gap-1">
                <svg className="w-2.5 h-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                {t('jobDetails.eligibility')}
              </span>
            )}
          </div>
          <h3 className="text-sm font-semibold text-gray-200 leading-tight group-hover:text-white transition-colors line-clamp-2">
            {examTitle}
          </h3>
          <div className="flex items-center text-[10px] text-gray-600 mt-1 gap-1.5 flex-wrap">
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          <button
            ...
            className={`p-1.5 rounded-lg border transition-all duration-150 ${isLiked ? 'bg-red-900/20 border-red-900/30 text-red-400' : 'bg-[#111] border-[#1e1e1e] text-gray-600 hover:text-red-400 hover:border-[#252525] hover:bg-[#141414]'}`}
          ...
          {(isLive || isRecentlyClosed || job.form_status === 'CLOSED') && (
            <button
              ...
              className={`p-1.5 rounded-lg border transition-all ${isApplied ? 'bg-blue-900/20 border-blue-900/40 text-blue-400' : 'bg-[#111] border-[#1e1e1e] text-gray-500 hover:text-blue-400 hover:border-blue-900/30 hover:bg-[#141414]'}`}
      ...
      <div className="grid grid-cols-2 gap-1.5 mt-3">
        <div className="bg-[#090909] rounded-md p-2 border border-[#141414]">
          <p className="text-[10px] text-gray-700">{t('job.age')}</p>
          <p className="text-xs text-gray-400 font-medium mt-0.5">{job.minimum_age}–{job.maximum_age} {t('job.years')}</p>
        </div>
        <div className="bg-[#090909] rounded-md p-2 border border-[#141414]">
          <p className="text-[10px] text-gray-700">{t('job.qualification')}</p>
          <p className="text-xs text-gray-400 font-medium mt-0.5 truncate">{job.qualification_required}</p>
        </div>
        {job.salary_max > 0 && (
          <div className="col-span-2 bg-[#090909] rounded-md p-2 border border-[#141414]">
            <p className="text-[10px] text-gray-700">{t('job.salary')}</p>
            <p className="text-xs text-gray-400 font-medium mt-0.5">₹{job.salary_min.toLocaleString('en-IN')} – ₹{job.salary_max.toLocaleString('en-IN')}</p>
          </div>
        )}
      </div>
      ...
      {(isLive || job.form_status === 'UPCOMING') && (
        <div className="flex items-center justify-between mt-2.5">
          ...
          {isLive && daysRemaining !== null && (
            <span className="text-[10px] font-bold text-red-500 bg-red-950/30 px-1.5 py-0.5 rounded border border-red-900/30 animate-pulse whitespace-nowrap">
              {daysRemaining === 0 ? "⚠️ Closing Today" : `⏳ Closes in ${daysRemaining} days`}
            </span>
          )}
          {job.form_status === 'UPCOMING' && daysUntilOpen !== null && (
            <span className="text-[10px] font-bold text-emerald-500 bg-emerald-950/30 px-1.5 py-0.5 rounded border border-emerald-900/30 animate-pulse">
              ⏳ {t('job.opensIn') || 'Opens in'} {daysUntilOpen}d
            </span>
          )}
        </div>
      )}
      {isRecentlyClosed && (
        <div className="flex items-center justify-between mt-2.5">
          ...
          {daysSinceClosed !== null && (
            <span className="text-[10px] font-bold text-orange-500 bg-orange-950/30 px-1.5 py-0.5 rounded border border-orange-900/30">
