...
export default function DashboardPage() {
  ...
  const [partialJobs, setPartialJobs] = useState<Job[]>([]);
  ...
  // Debug log after state is initialized
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const { search: sSearch, category: sCat, activeTab: sTab, viewMode: sView } = JSON.parse(saved);
  ...
  }, [search, category, activeTab, viewMode]);
      ...
      try {
        ...
        const [me, jobs, eligible, partial, liked, applied] = await Promise.all([
          api.getMe().catch(err => {
             ...
             return cachedMe || { full_name: 'Guest' };
          }),
  ...
  // Memoize all sub-arrays so tab switches don't re-filter 1500+ jobs each render
